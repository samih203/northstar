...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with tab2:
        ...
        if rows:
            ...
            st.plotly_chart(fig,use_container_width=True, key="thematic_cards")
            ...
            with st.spinner(f"Training model on {ticker_input}…"):
                ...
                if df.empty: st.error(f"No data found for {ticker_input}")
                    ...
                    if "error" in result: st.warning(result["error"])
                        ...
                        st.plotly_chart(fig2,use_container_width=True, key="ml_candle")
    ...
    with tab2:
        ...
        for pname,pdata in AI_WATCHLIST.items():
            ...
            with st.expander(f"📂  {pname}  ·  Risk: {risk}  ·  Horizon: {pdata['time_horizon']}",expanded=(pname=="High Conviction")):
                ...
                st.plotly_chart(fig,use_container_width=True, key="analysis_main")
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if not ticker:
        ...
        if df.empty or not q or q.get("error"): st.error(f"Could not load data for {ticker}")
            ...
            st.plotly_chart(fig,use_container_width=True, key="portfolio_perf")
            ...
            if "macd" in df_feat.columns:
                ...
                st.plotly_chart(fm,use_container_width=True, key="analysis_macd")
            ...
            if ctickers:
                ...
                st.plotly_chart(fc,use_container_width=True, key="analysis_compare")
