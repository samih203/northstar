...
CHART_THEME = dict(paper_bgcolor="#111827",plot_bgcolor="#0a0e1a",font=dict(color="#9ca3af",family="Space Mono, monospace",size=10),gridcolor="#1f2937")
...
if "Market Overview" in page:
    ...
    with col_left:
        ...
        if not df.empty:
            ...
            fig.update_layout(**CHART_THEME,height=280,margin=dict(l=0,r=0,t=10,b=0),showlegend=False,xaxis=dict(gridcolor="#1f2937"),yaxis=dict(gridcolor="#1f2937"))
    ...
    with col_right:
        ...
        if not sdf.empty:
            ...
            fig2.update_layout(**CHART_THEME,height=280,margin=dict(l=0,r=10,t=10,b=0),bargap=0.35,xaxis=dict(gridcolor="#1f2937"),yaxis=dict(gridcolor="#1f2937"))
    ...
    fig3.update_layout(**CHART_THEME,height=300,margin=dict(l=0,r=0,t=10,b=0),yaxis_title="Return %",xaxis=dict(gridcolor="#1f2937"),yaxis=dict(gridcolor="#1f2937"),legend=dict(bgcolor="#111827",bordercolor="#1f2937",borderwidth=1,font=dict(size=10,family="Space Mono, monospace")))
...
# ══════════════════════════════════════════════════════════════════════════════
        ...
        if run_scan or "screener_df" in st.session_state:
            ...
            if not df_f.empty:
                ...
                with c1:
                    ...
                    fig.update_layout(**CHART_THEME,height=200,margin=dict(l=0,r=0,t=10,b=0),xaxis=dict(gridcolor="#1f2937"),yaxis=dict(gridcolor="#1f2937"))
                    st.plotly_chart(fig,use_container_width=True)
                with c2:
                    top5=df_f.head(5)
                    fig2=go.Figure(go.Bar(x=top5["Score"],y=top5["Ticker"],orientation="h",marker_color="#00d4aa",text=top5["Score"],textposition="outside"))
                    fig2.update_layout(**CHART_THEME,height=200,margin=dict(l=0,r=0,t=10,b=0),xaxis=dict(range=[0,110],gridcolor="#1f2937"),yaxis=dict(gridcolor="#1f2937"))
                    st.plotly_chart(fig2,use_container_width=True)
    ...
    with tab2:
        ...
        if rows:
            ...
            fig.update_layout(**CHART_THEME,height=320,margin=dict(l=0,r=0,t=10,b=0),yaxis_title="Return %",xaxis=dict(gridcolor="#1f2937"),yaxis=dict(gridcolor="#1f2937"),legend=dict(bgcolor="#111827",bordercolor="#1f2937",borderwidth=1,font=dict(size=10,family="Space Mono, monospace")))
            st.plotly_chart(fig,use_container_width=True)
            ...
            with st.spinner(f"Training model on {ticker_input}…"):
                ...
                if df.empty: st.error(f"No data found for {ticker_input}")
                    ...
                    if "error" in result: st.warning(result["error"])
                        ...
                        fig2.update_layout(**CHART_THEME,height=320,margin=dict(l=0,r=0,t=10,b=0),xaxis=dict(gridcolor="#1f2937",rangeslider_visible=False),yaxis=dict(gridcolor="#1f2937"),xaxis_rangeslider_visible=False,legend=dict(bgcolor="#111827",bordercolor="#1f2937",borderwidth=1,font=dict(size=9,family="Space Mono, monospace")))
                        st.plotly_chart(fig2,use_container_width=True)
    ...
    with tab2:
        ...
        for pname,pdata in AI_WATCHLIST.items():
            ...
            with st.expander(f"📂  {pname}  ·  Risk: {risk}  ·  Horizon: {pdata['time_horizon']}",expanded=(pname=="High Conviction")):
                ...
                fig.update_layout(**CHART_THEME,height=250,margin=dict(l=0,r=0,t=10,b=0),yaxis_title="Return %",xaxis=dict(gridcolor="#1f2937"),yaxis=dict(gridcolor="#1f2937"),legend=dict(bgcolor="#111827",bordercolor="#1f2937",borderwidth=1,font=dict(size=9,family="Space Mono, monospace")))
                st.plotly_chart(fig,use_container_width=True)
    ...
    if not ticker:
        ...
        if df.empty or not q or q.get("error"): st.error(f"Could not load data for {ticker}")
            ...
            if "macd" in df_feat.columns:
                ...
                fm.update_layout(**CHART_THEME,height=200,margin=dict(l=0,r=0,t=10,b=0),xaxis=dict(gridcolor="#1f2937"),yaxis=dict(gridcolor="#1f2937"),legend=dict(bgcolor="#111827",bordercolor="#1f2937",borderwidth=1,font=dict(size=9,family="Space Mono, monospace")))
            ...
            if ctickers:
                ...
                fc.update_layout(**CHART_THEME,height=280,margin=dict(l=0,r=0,t=10,b=0),yaxis_title="Return %",xaxis=dict(gridcolor="#1f2937"),yaxis=dict(gridcolor="#1f2937"),legend=dict(bgcolor="#111827",bordercolor="#1f2937",borderwidth=1,font=dict(size=9,family="Space Mono, monospace")))
