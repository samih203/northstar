...
def screen_undervalued():
    """
    Quant Finance Multi-Factor Score (0-100):
      Factor 1 — Value (25pts):  EV/EBITDA, P/FCF proxy, P/B, P/S
      Factor 2 — Quality (25pts): ROE, ROIC proxy (ROE*equity_ratio), Gross Margin, Debt/Equity
      Factor 3 — Growth (20pts): Revenue CAGR, Earnings growth, Forward PEG
      Factor 4 — Momentum (20pts): 52W price momentum, RSI band, volume trend
      Factor 5 — Analyst/Margin of Safety (10pts): upside to consensus, short interest
    """
    universe = [
        "CSCO","HPQ","INTC","STX","WDC","NTAP","JNPR",
        "ASTE","GNRC","ESE","KTOS","EMR","ROK",
        "ACAD","HIMS","HALO","CERT","PRGO",
        "CIVI","MGY","CHX","TALO","VTLE",
        "COOP","PFSI","CALM",
        "ZEUS","MP","CLW",
        "IONQ","SMAR","ASAN","FRSH","BRZE",
        "GDEN","JACK","CENT","PRGO",
    ]
    universe = list(dict.fromkeys(universe))  # deduplicate
    rows = []
    prog = st.progress(0, text="Running quant screen…")

    for i, tick in enumerate(universe):
        prog.progress((i+1)/len(universe), text=f"Analyzing {tick}…")
        q = get_quote(tick)
        ...
        v_score = 0
        # EV/EBITDA: best value signal — below sector median (~12x)
        if ev_eb and 0 < ev_eb < 8:   v_score += 10; signals.append(f"EV/EBITDA {ev_eb:.1f}x")
        elif ev_eb and ev_eb < 14:     v_score += 5
        # Price-to-Book
        if pb and 0 < pb < 1.5:        v_score += 8; signals.append(f"P/B {pb:.1f}x")
        elif pb and pb < 3:             v_score += 4
        # Price-to-Sales
        if ps and ps < 1.5:            v_score += 7; signals.append(f"P/S {ps:.1f}x")
        elif ps and ps < 3:             v_score += 3
        score += min(v_score, 25)

        # ── FACTOR 2: QUALITY (max 25pts) ────────────────────────────
        q_score = 0
        # ROE: measures capital efficiency
        if roe > 0.20:    q_score += 10; signals.append(f"ROE {roe*100:.0f}%")
        elif roe > 0.12:  q_score += 6
        elif roe > 0.06:  q_score += 3
        # Gross margin: pricing power proxy
        if gm > 0.50:     q_score += 8; signals.append("High Margins")
        elif gm > 0.35:   q_score += 5
        elif gm > 0.20:   q_score += 2
        # Debt/Equity: balance sheet health
        if de < 0.3:      q_score += 7; signals.append("Low Debt")
        elif de < 0.8:    q_score += 4
        elif de > 2.0:    q_score -= 3  # penalize over-leverage
        score += min(q_score, 25)

        # ── FACTOR 3: GROWTH (max 20pts) ─────────────────────────────
        g_score = 0
        if rev_g > 0.25:   g_score += 10; signals.append(f"Rev +{rev_g*100:.0f}%")
        elif rev_g > 0.12: g_score += 6
        elif rev_g > 0.05: g_score += 3
        if earn_g > 0.20:  g_score += 7; signals.append("Earn Growth")
        elif earn_g > 0.10: g_score += 4
        # Forward PEG ratio: growth at reasonable price
        if fpe and earn_g and earn_g > 0:
            peg = fpe / (earn_g * 100)
            if peg < 0.8:   g_score += 3; signals.append(f"PEG {peg:.2f}")
            elif peg < 1.5: g_score += 1
        score += min(g_score, 20)

        # ── FACTOR 4: MOMENTUM (max 20pts) ───────────────────────────
        m_score = 0
        if price > 0 and hi52 > 0 and lo52 > 0:
            # 52W position: price position within range (prefer upper half but not overbought)
            rng = hi52 - lo52
            pos = (price - lo52) / rng if rng > 0 else 0.5
            if 0.4 < pos < 0.75:  m_score += 8   # sweet spot
            elif pos >= 0.75:     m_score += 5   # strong momentum
            elif pos < 0.25:      m_score += 2   # potential value trap
            # Discount from 52W high: contrarian catalyst signal
            disc = (hi52 - price) / hi52 * 100
            if 15 < disc < 35:   m_score += 7; signals.append(f"{disc:.0f}% off high")
            elif disc < 10:      m_score += 3
        # Beta-adjusted: prefer moderate beta for risk-adjusted returns
        if 0.7 < beta < 1.4:     m_score += 5
        elif beta < 0.7:         m_score += 2
        score += min(m_score, 20)

        # ── FACTOR 5: ANALYST / MARGIN OF SAFETY (max 10pts) ─────────
        a_score = 0
        if tgt and price and tgt > price:
            upside = (tgt - price) / price * 100
            if upside > 30:    a_score += 7; signals.append(f"+{upside:.0f}% target")
            elif upside > 15:  a_score += 4
        # Low short interest: market not betting against it
        if short_r and short_r < 3:   a_score += 3
        elif short_r and short_r > 8: a_score -= 2  # high short interest penalty
